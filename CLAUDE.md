...
- **Captcha is ON** (confirmed live 2026-09-14): Supabase rejects `signInWithOtp` without a
  Turnstile `captchaToken`; OAuth (Google) is exempt. The app satisfies it like web does:
  `lib/auth/captcha.tsx` (`CaptchaProvider` + `requestCaptchaToken()`) loads the web-hosted
  `/turnstile` page in a `react-native-webview` sheet and receives the token by postMessage;
  `components/passwordless-signin.tsx` passes it as `options.captchaToken`.
...
| Area | Target | On disk |
|---|---|---|
| Auth | Supabase | ✅ `lib/supabase.ts` (AsyncStorage, PKCE), `providers/supabase-auth-provider.tsx`, `services/auth.service.ts` + `lib/auth/email-auth.ts`: **auth screens rebuilt 2026-09-22** — `(auth)/sign-in` = Welcome (Create account / Sign in / Apple / Google), `create-account` (email+password → `signUp` → code), `log-in` (password, or "Email me a code instead" → `signInWithOtp`), `verify-code` (six cells, `verifyOtp` type signup|email), `forgot-password` (`resetPasswordForEmail` → the web's reset page). Every email request carries a Turnstile token: on iOS the check runs in a Safari sheet (`openAuthSessionAsync`, page returns `normalapp://captcha?token=`), Android keeps the WebView sheet (`lib/auth/captcha.tsx`). Google (PKCE via `expo-auth-session`, redirect `normalapp://wallet-setup`), **Apple (native `expo-apple-authentication` → `signInWithIdToken`, nonce-bound; `ios.usesAppleSignIn` entitlement; needs the Apple provider enabled in Supabase with both bundle ids as client IDs — UNVERIFIED until Niko checks the dashboard)**. `app/wallet-setup.tsx` is only a deep-link landing (redirects to tabs). **Onboarding is asset-first (web get-started, confirmed 2026-09-22)**: `app/create-wallet.tsx` = "What do you want to start with?" (USDC/XLM/BTC/ETH/SOL → Buy or Receive); the chain's wallet is created inside that flow with one passkey via `lib/turnkey/provision.ts` (`provisionChain`: no wallet → passkey + `POST turnkey/wallet { chain }` for THAT chain only; wallet exists → `ensureChainAddress`); "Skip for now" (`lib/onboarding.ts`, process-scoped) lands on a wallet-less Home and every Buy/Receive/Send/Swap/Savings entry point provisions on demand. No password step on the email-code path. **Recovery phrase built 2026-09-22**: `lib/turnkey/export.ts` (`EXPORT_WALLET` to an ephemeral P-256 key from `@turnkey/crypto`, `decryptExportBundle` locally, 12 words) + `app/backup.tsx` (shown right after the first wallet is created, from Home's Back-up card while unconfirmed, and Settings → Security → Recovery phrase); the checkbox is optional on mobile (Niko), marks are local only like web (`wallet_needs_backup_v1` / `wallet_backed_up_v1:<subOrgId>`). Referral activation after the first Stellar address is not wired. |
...
| Chains | BTC, ETH, SOL, XLM | ✅ Reads on all four. **Send on all four** (`app/send.tsx`): XLM/USDC client-side to Horizon (`lib/stellar/send.ts`), ETH via viem EIP-1559 + `SIGN_TRANSACTION_V2` + `POST send/execute` (`lib/send/evm.ts`), SOL via `@solana/web3.js` + raw ed25519 + `send/execute` (`lib/send/solana.ts`), BTC via `turnkey/build-btc-tx` PSBT + `TRANSACTION_TYPE_BITCOIN` + `broadcast-btc` (`lib/send/bitcoin.ts`); dust/rent rules `lib/send/native-dust.ts` (verbatim web); per-asset meta + QR (BIP-21 / EIP-681 / `solana:` / SEP-0007) in `lib/send/registry.ts` + `lib/stellar/qr.ts`. **Lazy chain accounts** `lib/turnkey/accounts.ts` (`ensureChainAddress`: passkey `CREATE_WALLET_ACCOUNTS` → `turnkey/import`), offered inline on Send/Buy. ⚠️ ETH/SOL/BTC sends are written from the web source and typecheck/bundle, but are **untested on a device** (2026-09-16). |
| Swap | Soroswap / LI.FI / CCTP | ✅ **Soroswap XLM↔USDC live on a device (2026-09-15)**: `lib/swap/soroswap.ts` (net-amount quote + `gross_amount`; embedded-fee one-signature `swap/submit-single` when the server carries our fee, in-place degrade to the shared fee-pair funnel on 409 `embedded_unavailable`), `app/(tabs)/swap.tsx` (gates in web order, Soroban-fee guards, 500ms debounced quotes only while on screen — `swap/quote` is 30/10s per IP). **CCTP outbound (USDC → BTC/ETH/SOL) built 2026-09-16, untested on a device**: `lib/cctp/` (config + verbatim phase table, `burn-stellar` approve+deposit_for_burn via Soroban RPC `EXPO_PUBLIC_MAINNET_RPC_URL`, `base` LI.FI pivot + delivery poll on Base RPC, `burn-evm` for refunds, `engine` = row-before-broadcast → burn → poll to COMPLETED → autopilot/pivot with one failover else top-up + passkey pivot → delivery; `recoverOutbound` / `refundOutbound`), `app/crosschain-swap.tsx` (entered from the Swap tab's "Swap to other chains"), `components/swap/InFlightTransfers.tsx` (history=1, phase table, pokes auto rows, Finish / Bring back). Activity shows one row per transfer. **CCTP inbound ETH/SOL → USDC built 2026-09-16** (`lib/lifi/execute.ts` LI.FI source leg — EVM re-priced to live gas, pre-sign affordability, nonce-race re-sign; SOL v0 signature spliced; `runInboundSwap`: row → source swap → Base arrival watch (95% of toAmountMin, LI.FI verdict every 3rd poll, calm endings) → autopilot burn else top-up + passkey burn with forwarder + hookData → poll to COMPLETED; `recoverInbound` from In flight; `components/swap/CctpInboundPanel.tsx`). **Autopilot** (`lib/turnkey/autopilot.ts`: CREATE_API_ONLY_USERS + CREATE_POLICY_V3, exact web policy string; consent sheet at swap start, mid-run "Enable auto-finish", Settings switch) makes both directions single-signature — requires `EXPO_PUBLIC_AUTOPILOT_PUBLIC_KEY` byte-identical to the server's key. Swap tab = from/to pickers routing by pair (`lib/swap/registry.ts`, web's `canPair`). **LI.FI native ⇄ native (BTC / ETH / SOL) built 2026-09-16, untested on a device**: `components/swap/LifiPanel.tsx` (web use-lifi-engine: $5 min, per-chain reserves BTC 0.00002 / ETH live gas / SOL 0.01, gas-share warn >20% block >50%, 10-min freshness + 1% drift), run kind `lifi` in `lib/swap/runner.ts` (one passkey → `lib/lifi/tracker.ts`: confirm source, `lifi/record` (5xx-only retries — the route has no dedupe), poll `lifi/status` for the destination's settlement window × 1.5, destination refetch before Done, session status overrides for the feed). **BTC as a source built 2026-09-16** (`lib/lifi/btc-sign.ts`: local BIP-143 sighashes, ONE `SIGN_RAW_PAYLOADS` batch with `HASH_FUNCTION_NO_OP`, low-S, spend verdict, `turnkey/broadcast-btc`; hard rule 8) — used by the LI.FI route and CCTP inbound BTC → USDC. Every swap runs on `app/swap-run.tsx` (explicit Start, live steps from `lib/swap/run-store.ts`; In flight reopens live runs and server rows). |
...
| `EXPO_PUBLIC_ETH_RPC_URL` | falls back to `https://ethereum-rpc.publicnode.com` (`lib/send/evm.ts`) — set the web's value |
| `EXPO_PUBLIC_SOLANA_RPC_URL` | falls back to `https://solana-rpc.publicnode.com` (`lib/send/solana.ts`) — set the web's value |
| `EXPO_PUBLIC_MAINNET_RPC_URL` | Soroban RPC for CCTP burns; falls back to web's default `https://rpc.lightsail.network/` — set the web's `NEXT_PUBLIC_MAINNET_RPC_URL` |
| `EXPO_PUBLIC_BASE_RPC_URL` | optional; prepended to the Base pool (`mainnet.base.org`, publicnode) |
| `EXPO_PUBLIC_AUTOPILOT_PUBLIC_KEY` | absent = autopilot dark (prompts instead); must equal the server's `AUTOPILOT_API_PUBLIC_KEY` |
| `APP_VARIANT` (build-time, `eas.json`) | `production` → `io.normalfinance.app`; else `.app.dev` |
...
- **Account-level blockers nobody can fix from code (2026-09-16):** (1) Supabase → Authentication →
  Providers → Apple enabled with `io.normalfinance.app` + `io.normalfinance.app.dev` as client IDs;
  (2) Coinbase CDP dashboard: add `normalapp://buy` to the on-ramp redirect allow-list (owner
  unknown — same account as `COINBASE_KEY_ID`); (3) Turnstile hostname; (4) Expo org.
- **Quote rate limit (both clients, raised by the web agent 2026-09-16):** `swap/quote` and `lifi/quote` are IP-keyed 30/10s; carrier NAT makes mobile users throttle each other. Ask web for per-user keying (Bearer when present, IP fallback). Optional: `lifi/record` dedupe on txHash.
