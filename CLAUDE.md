...
| Swap | Soroswap / LI.FI / CCTP | ✅ **Soroswap XLM↔USDC live on a device (2026-09-15)**: `lib/swap/soroswap.ts` (net-amount quote + `gross_amount`; embedded-fee one-signature `swap/submit-single` when the server carries our fee, in-place degrade to the shared fee-pair funnel on 409 `embedded_unavailable`), `app/(tabs)/swap.tsx` (gates in web order, Soroban-fee guards, 500ms debounced quotes only while on screen — `swap/quote` is 30/10s per IP). **CCTP outbound (USDC → BTC/ETH/SOL) built 2026-09-16, untested on a device**: `lib/cctp/` (config + verbatim phase table, `burn-stellar` approve+deposit_for_burn via Soroban RPC `EXPO_PUBLIC_MAINNET_RPC_URL`, `base` LI.FI pivot + delivery poll on Base RPC, `burn-evm` for refunds, `engine` = row-before-broadcast → burn → poll to COMPLETED → autopilot/pivot with one failover else top-up + passkey pivot → delivery; `recoverOutbound` / `refundOutbound`), `app/crosschain-swap.tsx` (entered from the Swap tab's "Swap to other chains"), `components/swap/InFlightTransfers.tsx` (history=1, phase table, pokes auto rows, Finish / Bring back). Activity shows one row per transfer. ❌ LI.FI same-group (BTC/ETH/SOL ⇄) and CCTP inbound not started (map in `docs/lifi-cctp-plan.md`). |
