...
export const AssetList: React.FC<AssetListProps> = ({
  ...
  const filteredAssets = React.useMemo(() => {
    ...
    switch (selectedCategory) {
      case "crypto":
        return assets.filter(asset => cryptoAssets.includes(asset.asset_code));
      case "dwms":
    ...
    <YStack space="$4">
      ...
      <XStack justifyContent="space-between" paddingHorizontal="$2">
        {categories.map((category) => (
          <Button
            key={category.value}
            size="$2"
            backgroundColor={selectedCategory === category.value ? "$gray4" : "transparent"}
            color={selectedCategory === category.value ? "$textPrimary" : "$textSecondary"}
            fontSize="$3"
            fontWeight="500"
            borderRadius="$3"
            paddingHorizontal="$2"
      ...
      <YStack>
        {filteredAssets.map((asset, index) => (
          <AssetItem key={`${asset.asset_code}-${asset.asset_issuer || 'native'}`} asset={asset} />
        ))}
