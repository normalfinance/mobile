...
**Date:** 2026-09-10
**Transcribed by the mobile session from the chat reply; encoding artefacts (arrows, ×, ≤) repaired, content otherwise verbatim.**

Every answer below was verified in code on 2026-09-10. Paths are relative to `packages/web/` unless
...
> **Mobile-session note (2026-09-10, later the same day):** the live AASA at
> `https://normalfinance.io/.well-known/apple-app-site-association` now returns
> `FA938A596N.io.normalfinance.app` and `FA938A596N.io.normalfinance.app.dev` — the Team ID
> placeholder has been filled since this answer was written. The Android SHA-256 in
> `assetlinks.json` is still all zeros.
